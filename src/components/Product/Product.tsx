  ...
  const Product: React.FC = () => {
    ...
    return (
        <section className="Product">
            <div className="ProductMainBlk"><p className="ProductMainText">Наша продукция</p></div>
          <div className="Row ProductContainer">
          {images.map((image, index) => (
            <ImageWithText key={index} imageUrl={image.imageUrl} text={image.text} />
          ))}
        </div>
        <div className="ProductButton"><ProductButton /></div>
        </section>
